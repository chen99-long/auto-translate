type Language = 'zh' | 'en' | 'ja' | 'ko' | 'fr' | 'de' | 'es' | 'it' | 'ru' | 'pt' | 'vi' | 'id' | 'th' | 'ms';
...
function getTranslateTemplate(textList: string[], from: Language = 'zh', to: Language = 'en'): TranslateTemplate {
  return {
    header: {
      fn: "auto_translation",
      client_key:
...
export async function translateAPI(text: string, options: TranslateOptions = {}): Promise<string> {
